...
namespace emoc {
	...
	void Global::SetProblem(Problem* problem)
	{
		problem_ = problem;
		SetDecBound();
		std::cout << "Set Customized Problem Successfully!\n";
		std::cout << "New Problem's Decision Bounds:\n";
		std::cout << "[";
		for (int i = 0; i < dec_num_; ++i)
			std::cout << dec_lower_bound_[i] << " ";
		std::cout << "]\n";
		std::cout << "[";
		for (int i = 0; i < dec_num_; ++i)
			std::cout << dec_upper_bound_[i] << " ";
		std::cout << "]\n";
	...
	void Global::SetDecBound()
	{
		for (int i = 0; i < dec_num_; ++i)
		{
			...
			//std::cout << dec_lower_bound_[i] << " " << dec_upper_bound_[i] << "\n";
		}
	}
