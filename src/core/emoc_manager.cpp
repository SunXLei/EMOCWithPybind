...
namespace emoc {
	...
	void EMOCManager::EMOCSingleThreadRun()
	{
		...
		for (int run = 0; run < para_.runs_num; ++run)
		{
			...
			// Check customized problem, this need to check before the Init() function
			...
			// for python side, omit these works for custom problem situtation
			if (g_GlobalSettingsArray[thread_id]->is_customized_problem_ == false)
			{
			...
			py_res.pop_num = g_GlobalSettingsArray[thread_id]->algorithm_->GetRealPopNum();
			py_res.runtime = g_GlobalSettingsArray[thread_id]->algorithm_->GetRuntime();
			for (int j = 0; j < py_res.pop_num; j++)
			{
				...
				{
					py_res.pop_decs[j][k] = g_GlobalSettingsArray[thread_id]->parent_population_[j]->dec_[k];
				}

				for (int k = 0; k < obj_num; k++)
				{
					py_res.pop_objs[j][k] = g_GlobalSettingsArray[thread_id]->parent_population_[j]->obj_[k];
				}
			}
